...
export default function Permohonan() {
  ...
  // Di dalam Permintaan.jsx, ganti fungsi handleSubmit-nya saja:
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    // Alamat ini harus SAMA PERSIS dengan yang di server.js dan routes
    // baseURL (/api) + path (/tugas/buat) = /api/tugas/buat
    const res = await api.post("/tugas/buat", dataKirim);
    ...
    // Log ini akan memberitahu kita jika ada error dari database
    console.error("DEBUG ERROR:", err.response?.data || err.message);
    alert("Gagal: " + (err.response?.data?.msg || "Cek koneksi database"));
  } finally {
    setLoading(false);
  }
};
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    <div className="relative group">
                      ...
                      <input 
                        ...
                        required 
                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 pl-12 pr-4 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium" 
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      value={form.sarana} 
                      onChange={handleChange} 
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all cursor-pointer font-medium appearance-none"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input 
                        ...
                        onChange={handleChange} 
                        required 
                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 px-4 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium" 
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      onChange={handleChange} 
                      placeholder="Contoh: Untuk keperluan presentasi rapat tahunan..." 
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium resize-none" 
                      required
                    ></textarea>
                  </div>
                  ...
                  <button 
                    type="submit" 
                    disabled={loading} 
                    className={`w-full ${loading ? 'bg-slate-400' : 'bg-blue-600 hover:bg-blue-700'} text-white font-bold py-4 rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2`}
                  >
                    <Send size={20} />
